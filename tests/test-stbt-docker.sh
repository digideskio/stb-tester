...
test_stbt_docker_fails_with_no_test_pack() {
    ...
    ! stbt docker exec true || fail
}

test_stbt_docker_exec_runs_command_with_no_setup_script() {
    load_test_pack empty-test-pack
    stbt docker exec echo -n hello >output || fail "Command should have succeeded"
...
test_that_stbt_docker_exec_fails_with_bad_test_pack()
{
    ...
    ! stbt docker exec true || fail "Command should have failed"
}

test_your_files_are_available_in_stbt_docker() {
    load_test_pack empty-test-pack
    stbt docker exec test -e config/stbt.conf || fail "Command should have succeeded"
}

test_that_path_within_container_is_same_as_outside() {
    load_test_pack empty-test-pack

    stbt docker exec bash -c '[ $PWD = /var/lib/stbt/test-pack ]' \
    || fail "Directory doesnt match"

    cd config
    stbt docker exec bash -c '[ $PWD = /var/lib/stbt/test-pack/config ]' \
    || fail "Directory doesnt match"
}
...
test_that_docker_opts_passes_arguments_through_to_docker_run() {
    ...
    stbt docker exec bash -c 'echo -e $HELLO >output'
